...
| `telemetry-sensors.md` | How the flight controller's custom telemetry becomes sensors on the radio: that every sensor the Telemetry page offers is decoded, that a sensor the suite does not recognise stops the frame and costs everything packed behind it, the two sensors the suite computes itself from the pack and why `SmFt` is not `Bat%`, and the two frame counters the decoder adds. | written | written |
