...
| `INVALID` | red | The radio has no sensor of that name at all — or, for a row that needs more than one, not all of them. | Let the radio discover the sensors: power the model up with telemetry running, then use *Discover new* on the radio's own Telemetry page. |

Three points worth knowing before you read a row:
...
- **One row needs two sensors.** *Adjustment Function* reads `AdjF` and `AdjV` together. It is
  `OK` only when both are current; `NO DATA` when the radio has both and neither is arriving,
  which is their ordinary state, since the pair only moves when an adjustment fires; and
  `INVALID` as soon as one of the two is missing from the radio, because waiting will not
  produce it and discovery will.
