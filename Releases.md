...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
  ...
  - Two trim layouts, per model: one trim per row (the documented six-band layout), or a walk-and-adjust pair in which one trim steps through the whole parameter set across banks and one moves the selected parameter.
  - Touch controls in fullscreen (bank chips, row selection, momentary step buttons with a plain-button fallback), and a separate ground screen reachable from the Quick Settings menu that carries the setup check and the actions below. The setup check walks the model once per visit to that screen and again only when the settings change; the in-air surface shows the verdict it reached and walks nothing.
