...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
  ...
  - Added **Set up the flight controller** to the settings page, offered in the standard set layout only. It reads the board first -- the receiver map, the function id of every slot, and the full record of every slot it would touch that is not already empty, never the whole-table read that overruns the flight controller's own buffer -- and builds a plan: what each slot holds now and what it would hold, how many are already in use, and which of them belong to a switch on a channel other than the two this model devotes to the overlay, which is the case that costs a pilot a control he flies with. Only on confirmation does it write, one MSP_SET_ADJUSTMENT_RANGE per slot chained from the previous reply and then a single EEPROM write, and it then reads every written slot back and compares it field by field. The thirty documented slots are written to the slot indices the documentation gives them, with the documented step and bounds, so a written board is that layout line for line. It refuses before the first message and abandons mid-chain when the model is armed, when the arming state cannot be read at all, when there is no link, and when the flight controller is too old to answer the paged reads.
