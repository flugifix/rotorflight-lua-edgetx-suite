...
- **Set up the model writes only what differs, and counts only what it can read back (`widgets/dashboard/inflight/setup.lua`, `app/pages/settings/dashboard/inflight/page.lua`)**:
  - The action was never a comparator: it appended both global variable writes unconditionally, queued every existing line on both channels for deletion and one insertion on each, and only the trim block compared anything. A model that already carried exactly the setup -- which the check two rows above the button reported as correct -- therefore got the same *remove one, add, set* question on every press, and the write deleted the very line its own last run had written. The plan now skips a variable whose name, range, precision and unit already match and a channel that already carries nothing but the line the overlay needs, and where nothing is left it says so instead of putting a confirmation up for a write with no content.
  - The setup check reads three more fields of the mixer line: an offset, a curve and the flight-mode mask. A line disabled in a flight mode is a fault, since a press moves nothing at all in that mode, and the mask is of the modes the line is disabled in -- so anything but zero is reported.
  - The trim half of the plan could never fix a trim fault. It resolved the claimed trims without the radio's trim block, and a stored trim setting is a switch position rather than the semantic number the firmware's trim-mode table is indexed by, so it looked every fault up under a key that does not exist. The plan is now handed the same resolved block the check gets.
  - A write is counted only once it has been verified. The Lua model writers do not raise on a refusal -- `insertMix` and `deleteMix` return nothing and do nothing when the mixer table is full or an index is out of range, and `setFlightMode` answers 0 or 2 -- so *Model set up (n)* used to count calls that had not raised. The firmware's own answer is returned where it gives one, the model is read back where it does not, and a write that cannot be read back is reported as not made.
  - A global variable other than the two configured ones carrying one of the two names the setup writes, `VAL` or `BNK`, is now named on the page. It is the only trace a leftover from an earlier pair leaves, since nothing refers to it any more and the reference walk beside it can only find references. A warning, not a refusal.
  - The check's two most dangerous verdicts, a bank and a value pointed at one variable or at one channel, each read as *Switch or variables not set* and now say what they are. The confirmation also states that it writes the values as they stand on the page, saved or not.
