...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
  ...
  - The surface itself is laid out for reading at arm's length: a header naming the set, the profile and its backup with a live marker, six bank chips carrying the standard set's own letters, the selected parameter large on the left with its row, bank and trim beneath it and its value larger still in the accent colour, what it was primed at and whether the announcement was heard beside it, and on the right the six rows with their number, the trim that drives them under the radio's own name for it, their name and their value. The layout is computed from the widget's own zone rather than from fixed numbers, so it lands the same way on 480x272, 480x320 and 800x480 screens.
  ...
  - The page now proposes the two global variables instead of leaving them at zero: on entry it walks every channel's mixer lines and every input's lines -- both encode a source reference the same way -- and offers the first two numbers none of them refers to as a weight or an offset, value variable first. The proposal is shown in the fields and is written only when the settings are saved, and where fewer than two are free the setup check says so.
  ...
  - After the flight the ground screen lists what changed: every parameter whose value has moved away from that snapshot, largest change first, capped to what the screen holds with the rest counted. The list is drawn disarmed only, and where there is no snapshot to compare with it says so instead of reporting an empty comparison as no change.
