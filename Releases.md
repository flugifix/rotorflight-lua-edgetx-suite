...
- **In-Flight Tuning (preview) (`widgets/dashboard/inflight`, `settings/dashboard/inflight`, `setup/controls/inflight`)**:
  ...
  - Added `Settings > Dashboard > In-Flight Tuning` for the radio's half -- interlock switch, enable and value channels and global variables, pulse length and trims -- with **Set up the model**, which builds a plan of the mixer lines, variable details and trim modes it would change, puts it up for confirmation and only then writes, in the order that keeps every intermediate state safe. The page reads the model back and reports what is missing, including a bank and a value pointed at one variable or one channel and one trim claimed by two rows.
  - Added `Setup > Controls > In-Flight Tuning`, beside Adjustments, for the flight controller's half -- set layout, step size and backup PID profile, with **Set up the flight controller**, which reads the board in two replies, puts up what each slot holds and what it would hold, writes one adjustment range per slot on confirmation followed by a single EEPROM write, and reads every written slot back field by field. The page is gated on the link and locked while armed like its neighbours, and each of the two pages names the other.
  - Added a per-model **step size** of 1, 2, 5 or 10, which is what one press moves a parameter by on the board. It is written into every slot the setup action puts there, so a changed step size means the flight controller has to be set up again, which the board comparison reports by name.
...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`, `setup/controls/inflight`)**:
  - Added an in-flight tuning surface to the dashboard widget: while a pilot-chosen interlock switch is on, the widget shows one adjustment parameter at a time with its bank, its row list and its live value, and drives the flight controller's own adjustment functions by pulsing two declared EdgeTX global variables onto the enable and value channels.
  - The interlock switch is the only entry, and what it shows follows the FLIGHT rather than the switch: on the ground before a flight the preflight read-out, in the air the tuning surface, and on the ground after a flight that moved something the list of what changed and the undo. Nothing steps on the ground -- the step controls and the trims are live in the air and nowhere else, because the ground surface carries neither a row list nor a step control.
  - Works while armed by construction: everything the overlay does in the air is a global variable write, so it is unaffected by the armed queue clear. The MSP half is a separate module and runs on the ground only, and refuses while the flight controller is armed, while the rotor is turning, and while the arm sensor has not yet been read at all -- a model whose arm sensor is not in the telemetry list reads as disarmed for ever, so that case is refused and named on screen rather than treated as safe.
  - The value variable is taken back to zero on the exit paths the widget sees (interlock released, fullscreen closed, widget backgrounded, link lost), and on three the widget does not: a held step control is let go before the rebuild that would delete the button reporting its release, the widget's render key does not move at all while a control is held, and the entry point can clear both variables directly while the widget is backed off after an instruction-limit fault -- during which none of the paths inside the widget runs. Each write is recorded before it is made, so a pass killed between the write and the note of it still leaves a clear to be made rather than a value nothing takes back.
  - Both variables are cleared in the flight mode they were written in, since the radio resolves a "same as FMx" link on the write itself.
  - The trims are chosen with the radio's own switch picker, filtered to the trim block, rather than from a list the page wrote: the picker offers exactly the trims this radio has and names each of them the way every other page of the radio does. The store keeps the switch position of the trim's increment and the overlay derives the decrement from the same block; a store written before this holds a semantic index and is converted once the radio has said what its trims are, in memory by the widget and on the card by the settings page's next save.
  - Two trim layouts, per model. **Walk and adjust**, which is the default, claims up to three trims: one steps the bank, one steps the row inside it, and one moves the selected parameter. Leave the bank trim off and the walk trim walks the whole set across banks as before. The alternative is one trim per row, the documented six-band layout, which needs six trims and so cannot be used on a radio that has four. The defaults are rudder to walk, elevator for the bank and aileron to adjust; a model whose settings already name a layout or a trim keeps what it names.
  - Every trim press is a PULSE and no longer a level. The flight controller counts no step until the value channel has stood still inside one window for its trigger delay, so a press shorter than that used to move the channel out and back without ever standing still and stepped nothing at all, while a press held for a second stepped as many times as the board's repeat delay fitted into it. A press now writes the row's magnitude for at least the configured step length however early the trim is let go, a trim held keeps it written for as long as it is held, and two taps the board could not tell apart produce one step rather than an arbitrary number.
  - The board's report and the trims are read on every widget pass rather than on the dashboard's slower logic tick, so the value on the screen follows the flight controller step by step and a short trim press cannot fall between two samples. The cost while the interlock is open is one table lookup.
  - The overlay follows the PID and rate profiles the flight controller is flying. The firmware's adjustment functions act on the active profile, and the pilot chooses it with his own switch, so a profile change now marks every cached value belonging to that profile as unknown at once, and names the new profile on the screen for three seconds so that a screenful of dashes says why it is a screenful of dashes. The acc trims, the governor configuration, the battery profile and the profile indices themselves are global and are kept. On the ground the nine value reads are sent again once the profile switch has stood still for two seconds -- a pilot moving through three profiles used to cost six rounds of nine reads -- and only those, since a slot table describes a layout and a layout does not move when a profile does; in the air nothing is read and the values stay unknown until the board reports each one. A backup records the profile it was taken from, the ground screen names it, and a restore into a different profile is refused rather than silently overwriting a profile the copy never described.
  - Touch controls in fullscreen (bank chips, row selection, momentary step buttons with a plain-button fallback), and a separate ground screen reachable from the Quick Settings menu that carries the setup check and the actions below. The setup check walks the model once per visit to that screen and again only when the settings change; it is shown on the ground screen only, since it describes a model that cannot change while the pilot is flying it.
  - The surface itself is laid out for reading at arm's length: a header whose headline is the ACTIVE PID profile -- the firmware's adjustments act on that one and everything on the surface is true of it and of no other -- with the phase and the backup beside it, six bank chips carrying the standard set's own letters, the selected parameter large on the left with its value larger still in the accent colour, and on the right the six rows with their number, the trim that drives them under the radio's own name for it, their name and their value. Beside the value stand the two numbers that make it mean something: where the parameter was a step ago, and where it started before the flight. The layout is computed from the widget's own zone rather than from fixed numbers, so it lands the same way on 480x272, 480x320 and 800x480 screens.
  - A step control that refuses now says so. Two presses closer together than the flight controller can tell apart are one step, which is right -- but the second press used to write nothing, log nothing and look exactly like a control that is not wired up.
  - Added `Settings > Dashboard > In-Flight Tuning`: interlock switch picker, enable/value channels and global variables, step length, trim layout and per-row trim assignment, and a backup PID profile, all stored per model. The page reads the mixer lines, the variables' details and the active flight mode's trim modes and reports what is missing -- including a bank and a value pointed at one variable or one channel, and one trim claimed by two rows, both of which leave the pilot moving a parameter other than the one on the screen.
  - The store now holds a working setup before a pilot has touched it: the value variable is GV5, the bank variable GV6, the undo the last PID profile, and the pulse 250 ms -- long enough to clear the board's 100 ms trigger delay and short enough to stay inside its 200 ms repeat, which is exactly one step. A model store that already names any of them keeps what it names. The model walk that used to CHOOSE the two variables now warns instead: it names the variable and the mixer or input line that already refers to it, and refuses nothing.
  - Added **Set up the model**, which makes the model match the settings after showing what that costs. It builds a plan first -- the lines currently on the enable and value channels, named as the mixer page names them and counted per channel, the two lines that would replace them, the two variables' names, range and precision, and the flight modes whose claimed trims are still live -- and puts it up as a confirmation. Only on confirmation does it write, in the order that keeps every intermediate state safe: variables, then the channel emptied (which parks it at centre), then the overlay's line, then the trim modes. Declining writes nothing, no channel other than the two configured is touched, every call is separate and its result recorded, and the setup check is taken again afterwards.
  - The settings page no longer loads the widget's live drive to reach the settings, the setup check and the trim names: those moved to `widgets/dashboard/inflight/setup.lua`, which the drive re-exports. The page's entry cost falls accordingly, which matters on a radio whose tool is near its heap ceiling -- the page as it stood could not be opened there without the tool running out of memory.
  - On the ground the overlay primes itself over MSP: the receiver map, the board's own adjustment slot table (the function ids in one reply, then one record per configured slot) and nine value reads that between them answer every adjustment function. The bank and row layout is then the board's own, derived from the enable and increment windows of the slots that sit on this model's declared channels, with the documented layout kept as the fallback and the screen saying which of the two is in use. Continuous slots are skipped rather than placed, since they have no park position to return to, and how many were skipped is on the screen beside the layout the set came from.
  - The prime runs once per connect, or on demand from the ground screen, and never while armed: it refuses to start, abandons a run that is armed into, and reads the queue's own clear as that abandon rather than as a failure.
  - Undo before the flight: the ground screen copies the flying PID profile into a spare one and commits it, and restores it afterwards, using the same MSP pair as the Copy Profiles tool. The refusals are explicit and shown under the button that was pressed (armed, no spare profile chosen, the same profile, past the board's profile count, no link, and the board not yet read), and the values the profile held are snapshotted with the copy, because the board writes an in-flight change into its own storage shortly after disarm. A backup before the board has been read is refused rather than taken, since it would snapshot nothing and the flight would then be reported as having changed nothing.
  - After a flight that moved something the overlay shows what changed, on a screen of its own and in full: every parameter whose value has moved away from that snapshot, largest change first, paged with the same trim that walks the parameters in the air, with a footer saying that the flight controller has already written all of it to its own storage. There is no cap: the entries a cap would have hidden are, on a board that has already saved them, exactly the ones nobody would ever see again. Where there is no snapshot to compare with it says so instead of reporting an empty comparison as no change. The screen ends when the interlock is switched off, when a new backup is made or on the next arming; the list itself stands until the next backup replaces the snapshot it is measured against.
  - The backup is made without being asked for, when the interlock is switched on while the machine is on the ground with the link up and the values read -- once per session and per active PID profile. It is deliberately not taken on the switch alone: an interlock switched on in the air would otherwise take its backup on landing and overwrite the undo with the values the flight had already moved. Every refusal the button applies applies here too and is shown in the same place.
  - Added a per-model **Set layout** setting with two modes. In **Standard**, which is the default, the six banks of six parameters are known in advance: they are the layout the project's own generic radio setup documents, with the six cells that layout leaves empty filled in (yaw inertia precompensation gain and yaw precompensation cutoff beside the F gains, the two cross-coupling terms beside the O gains, head speed and the tail-torque gain beside the governor's own). The screen names every parameter before the flight controller has said anything, and the board's own adjustment slot table is read only to be held against that set -- the surface then says whether the board matches it, differs in so many slots, or carries none of it. In **Custom** the behaviour is exactly what it was: the set is whatever the board carries, derived from its own enable and increment windows, and nothing is ever written to it.
  - Added **Set up the flight controller** to the settings page, offered in the standard set layout only. It reads the board first -- the receiver map and the function id of every slot, which is two replies and never the whole-table read that overruns the flight controller's own buffer -- and builds a plan: what each slot holds now and what it would hold, and which of them hold a different function today, named so that a pilot can recognise a control he flies with. The plan says plainly that only the function of each slot was read: which channel a slot watches, through which window, is checked after the write rather than before it, which is where that answer matters and is worth a round trip. Only on confirmation does it write, one MSP_SET_ADJUSTMENT_RANGE per slot chained from the previous reply and then a single EEPROM write, and it then reads every written slot back and compares it field by field. The thirty documented slots are written to the slot indices the documentation gives them, with the documented step and bounds, so a written board is that layout line for line. It refuses before the first message and abandons mid-chain when the model is armed, when the arming state cannot be read at all, when there is no link, and when the flight controller is too old to answer the paged reads.
  - The flight controller half of the settings page is loaded only when its button is pressed, for the same reason the setup check moved out of the widget's drive: everything a page's chunk holds is paid on entry, on a radio whose tool has already run out of heap once on this page.
  - Added `Setup > Controls > In-Flight Tuning`, beside Adjustments, and moved the flight controller's half of the settings there: the set layout, the step size, the backup PID profile and **Set up the flight controller**. `Settings > Dashboard > In-Flight Tuning` keeps the radio's half -- the interlock switch, the channels, the variables, the pulse length and the trims. They are not one job: the radio's half is true of the radio whether or not anything is connected, while the other cannot be looked at without a flight controller, so it is gated on the link and locked while armed like its neighbours. Every setting stays in the same per-model store section and each page carries a line naming the other.
  - Added a **step size** setting, 1, 2, 5 or 10, which is what one press moves a parameter by on the board. It is written into every slot the setup action puts there, so a changed step means the flight controller has to be set up again -- which the help says and which the board comparison reports by name rather than as one more slot that differs.
  - Fixed: a flight controller with no adjustment slots configured at all left the ground half waiting for a slot record that was never going to be asked for, so the prime never finished and never failed. That is exactly the board this feature exists to offer a set to.
  - Added offline instruction accounting rows `pass.tuning.state`, `pass.tuning.prime` and `pass.job.tuning` with measured figures, and the switch, global variable, mixer and flight-mode surface the accounting stubs needed for them. The ground half has a row of its own because the pass an MSP reply arrives in pays that reply's parse, and at most one reply is parsed per pass.
  - The surface is no longer rebuilt once per MSP reply while the board is being read. Every reply used to move the drive's value epoch, the epoch is in the widget's render key, and the tuning surface is forty-five objects, so reading a board tore the whole tree down and built it again beside every reply that was being parsed -- which is the one kind of overrun no log in the tool can see, since the firmware sweeps the built tree's reactive references outside the pcall the widget entry point wraps its refresh in. The counters travel on the published snapshot instead and the surface reads them through a reactive text closure: measured on the widget itself, one rebuild as a read starts and none at all over the forty-four replies after it. The read button refuses a second press while one is on the wire.
  - The ground half also stands aside on a pass that is already expensive: it parses nothing when the previous pass used more than seventy per cent of the instruction budget, nor on a pass that is about to build, and takes a slice anyway after a hundred such passes so that a busy widget cannot stall a read for ever. On a radio with a heavy theme the automatic read after a connect lands on start-up passes that are already near the limit.
  - A module whose own dependency did not load no longer returns at all. The suite's module loader runs a chunk under `pcall` and caches whatever it returns, and a `pcall` catches the firmware's instruction-limit error like any other -- so a pass that ran out of budget while the adjustment function table was being read left the drive cached with a nil reference and every later pass raising `attempt to index a nil value`, for the rest of the session, with the widget's refresh abandoned each time. Nothing is cached now and the next pass loads the file again; and the widget's own loaders no longer latch a failed load for the session either, but retry it a second later.
  - The flight controller read shows how far it has got, and is given up when the page is left. The progress line is a reactive closure rather than a rebuild per record -- a rebuild of that page allocates about fifteen kilobytes, and the read used to ask for one per slot on a radio with little heap to spare. A read the pilot walks away from is taken out of the queue by client id; a WRITE is not, because a write stopped half way leaves the flight controller holding part of one adjustment set and part of another, and the screen says so while it runs.
  - A changed setting now reaches the overlay without a radio restart. The widget's own preference watcher fires correctly, but it re-reads the per-model store through a loader that serves a module-level cache with no invalidation, and the tool that wrote the file runs in its own Lua state -- so what came back to the widget was a fresh table holding the file as it stood at connect, which defeats every identity test downstream of it. The overlay therefore watches the file itself: one `fstat` a second while the feature is enabled, and on a change one read that is forced past that cache. Nothing else's state is touched. The stamp is compared rather than consumed, so a pass killed between reading it and settling the drive loses nothing -- the next pass sees the same difference and settles then.
  - The German texts of the overlay are written with their proper umlauts.
  - The ground screen says what to do and in what order, and its refusals are spelled out. Where no backup profile has been chosen the two profile buttons read `--` rather than `0`, and the line under them says where to choose one; the help adds the order the screen is used in, and what reading the flight controller costs and when it is needed.
  - The overlay is a **preview feature** and is hidden until it is switched on under `Settings > General > Preview`. It is not finished -- its screens, its settings and what it writes to the flight controller can still change, and it is the one feature in the suite that moves the board's own parameters while the model is in the air -- so it is opt-in per feature, with the question asked once and the value moving only on yes. One key covers every way in: both menu entries carry it as their visibility condition, the dashboard widget asks it beside the per-model setting and builds no drive and reads no file of the overlay without it, and the Quick Settings offer the way into the surface only while it is on. The widget adopts a change to the switch when the preferences are next re-read, which does not happen while the craft is armed, so a switch thrown in the air takes effect after the landing.
  - Fixed: the ground screen reported `Setup not checked` for a whole session whenever the interlock was closed on the ground -- which is exactly what that screen is for. The check that walks the model was held off by the interlock rather than by the flight; it is held off by the flight now, and the ground pass, where a model read is legal, pays for it once.
  - The ground screen's four status lines take the row list's own scaling instead of the smallest font at a fixed pitch, which on an 800-pixel screen left them cramped under a header twice their height; each line is cut to the font it is drawn in rather than wrapped. In a widget zone the hint that a long press opens the touch controls is now shown whatever else stands above it, including the two states -- armed, and the arm sensor never seen -- in which a pilot is most likely to be looking for it.
