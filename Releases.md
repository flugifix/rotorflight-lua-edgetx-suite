...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
  ...
  - Works while armed by construction: everything the overlay does in the air is a global variable write, so it is unaffected by the armed queue clear, and the value variable returns to zero on every exit path (interlock released, fullscreen closed, widget backgrounded, link lost). The MSP half is a separate module and runs on the ground only.
  ...
  - Touch controls in fullscreen (bank chips, row selection, momentary step buttons with a plain-button fallback), and a separate ground screen reachable from the Quick Settings menu that carries the setup check and the actions below.
  - Added `Settings > Dashboard > In-Flight Tuning`: interlock switch picker, enable/value channels and global variables, step length, trim layout and per-row trim assignment, and a backup PID profile, all stored per model. The page reads the mixer lines, the variables' details and the active flight mode's trim modes and reports what is missing.
  - The page now proposes the two global variables instead of leaving them at zero: on entry it walks every channel's mixer lines and offers the first two numbers no line refers to as a weight or an offset, value variable first. The proposal is shown in the fields and is written only when the settings are saved, and where fewer than two are free the setup check says so.
  ...
  - Undo before the flight: the ground screen copies the flying PID profile into a spare one and commits it, and restores it afterwards, using the same MSP pair as the Copy Profiles tool. The refusals are explicit (armed, no spare profile chosen, the same profile, past the board's profile count, no link), and the values the profile held are snapshotted with the copy, because the board writes an in-flight change into its own storage shortly after disarm.
  - After the flight the ground screen lists what changed: every parameter whose value has moved away from that snapshot, largest change first, capped to what the screen holds with the rest counted.
  - Added offline instruction accounting rows `pass.tuning.state` and `pass.job.tuning` with measured figures, and the switch, global variable, mixer and flight-mode surface the accounting stubs needed for them. The tuning state row was re-apportioned once the ground half landed: the pass an MSP reply arrives in pays that reply's parse.
