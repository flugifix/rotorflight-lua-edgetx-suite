...
- **In-flight tuning: custom adjustment windows**: Plus, minus and trim inputs now use the selected bank and row's actual increment and decrement windows from the flight controller. Non-standard or asymmetric windows no longer drive the standard row ladder. Custom steps are refused until their windows are read, and a missing or unreachable window does not fall back to another parameter's value. The Standard layout is unchanged.
