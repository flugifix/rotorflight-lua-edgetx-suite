...
- **In-Flight Tuning Overlay (`widgets/dashboard/inflight`, `settings/dashboard/inflight`)**:
  ...
  - Works while armed by construction: the overlay speaks no MSP at all, so it is unaffected by the armed queue clear, and the value variable returns to zero on every exit path (interlock released, fullscreen closed, widget backgrounded, link lost).
  ...
  - Touch controls in fullscreen (bank chips, row selection, momentary step buttons with a plain-button fallback) and a read-only surface with the setup check reachable from the Quick Settings menu on the ground.
