...
local function decControl(data, pos)
    ...
    setTelemetryValue(0x1033, 0, 0, yaw, UNIT_DEGREE, 1, "CYaw")
    setTelemetryValue(0x1034, 0, 0, coll, UNIT_DEGREE, 1, "CCol")
    return nil, pos
end
...
local function decAttitude(data, pos)
    ...
    setTelemetryValue(0x1103, 0, 0, yaw, UNIT_DEGREE, 0, "Yaw")
    return nil, pos
end
...
local function decAccel(data, pos)
    ...
    setTelemetryValue(0x1113, 0, 0, z, UNIT_G, 1, "AccZ")
    return nil, pos
end
...
if type(requestedSensorsById) == "table" then
    -- Wird durch EdgeTX Sensor Discovery Feature aufgerufen
    initializeSensors(requestedSensorsById)
    return getSensorsBySid(requestedSensorsById)
else
    -- Wird durch rfsuite (telemetry_bg Task) aufgerufen
    local result = {}
    for id, sensor in pairs(sensorsById) do
        if id ~= 0 then result[sensor.sid] = { name = sensor.name, unit = sensor.unit, prec = sensor.prec, dec = sensor.dec } end
    end
    return result
end
