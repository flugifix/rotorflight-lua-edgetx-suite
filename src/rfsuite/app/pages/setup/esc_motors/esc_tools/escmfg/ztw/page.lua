...
local function ensureDeps()
  ...
  if not EscParametersZtwApi then EscParametersZtwApi = loadModule("tasks/msp/api/esc_parameters_ztw.lua") end
  if not LoadingOverlay then LoadingOverlay = loadModule("ui/loading_overlay.lua") end
  if not ConfirmDialog then ConfirmDialog = loadModule("ui/confirm_dialog.lua") end
  if not ZtwInit then ZtwInit = loadModule("app/pages/setup/esc_motors/esc_tools/escmfg/ztw/init.lua") end
...
local function queueZtwReadActual(queue)
  queue:add({
    command = EscParametersZtwApi.command,
    timeout = 15,
    simulatorResponse = EscParametersZtwApi.simulatorResponse,
    processReply = function(self, buf)
      local parsed = EscParametersZtwApi.parse(buf)
      if parsed then
...
local function queueZtwRead(isAutoReload)
  if not MspRuntime or not EscParametersZtwApi or type(MspRuntime.getState) ~= "function" then
...
local function queueZtwWrite(requestRebuild)
  if not MspRuntime or not EscParametersZtwApi or type(MspRuntime.getState) ~= "function" then
    return false, "msp_runtime_unavailable"
  end
  ...
  queue:add({
    command = EscParametersZtwApi.writeCommand,
    timeout = 5,
    maxRetries = 1,
    payload = EscParametersZtwApi.buildWritePayload(writeData),
    isWrite = true,
    processReply = function(self, buf)
      ui.dirty = false
      ui.saving = false
...
local function loadFromSession()
  local session = getSession()
  local cached = session and session.setup_esc_motors_esc_tools_ztw or nil
...
function M.onSave(ctx)
  local ok, err = queueZtwWrite(ctx and ctx.requestRebuild)
  if not ok then
...
function M.onClose()
  ...
  if closingSession then closingSession.setup_esc_motors_esc_tools_ztw = nil end
