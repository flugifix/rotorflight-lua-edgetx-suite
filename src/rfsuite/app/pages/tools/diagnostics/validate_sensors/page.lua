...
local function buildRows()
  ...
  for i = 1, #ids do
    ...
    if type(sources) == "table" and #sources > 0 then
      -- The middle state is decided by whether the radio has ALL of the row's sensors, not by
      -- whether one of them happens to be current: a multi-source row whose sensors are all
      -- there but idle -- the adjustment pair only moves when an adjustment fires -- is the
      -- same "known, nothing arriving" case as a single source, and calling it INVALID would
      -- say the radio does not have them. One of the pair missing is the other case: no amount
      -- of waiting produces it, so the row reads INVALID and sends the pilot to discovery.
      local allLive = true
