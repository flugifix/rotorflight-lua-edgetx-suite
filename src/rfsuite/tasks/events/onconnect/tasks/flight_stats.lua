-- OnConnect task: flight_stats auslesen (nur fuer Widget-Kontext)
local M = {}
...
function M.wakeup(args)
  ...
  if not requestSent and Log and type(Log.emit) == "function" then
    pcall(Log.emit, "rfsuite.tasks.flight_stats", "onconnect flight_stats.wakeup called, context=" .. tostring(context), "debug", true)
  end
