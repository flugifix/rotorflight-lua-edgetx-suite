-- Lightweight onarm tasks runner (similar to onconnect)
local M = {}
...
local function loadManifest()
  ...
  if type(chunk) ~= "function" then
    ensureLog()
    if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", "manifest missing: " .. tostring(MANIFEST_PATH), "debug", true) end
    tasksLoaded = true
    return
  end
  ...
  if not ok or type(manifest) ~= "table" then
    ensureLog()
    if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", "invalid manifest: " .. tostring(MANIFEST_PATH), "debug", true) end
    tasksLoaded = true
    return
  end
...
function M.wakeup()
  ...
  if isQueueDone() then
    if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", "all onarm tasks complete", "debug", true) end
    return
  end
  ...
  if not module then
    ...
    if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", "failed to load task " .. tostring(task.name) .. ": " .. tostring(err or "?"), "info", true) end
  ...
  if module.isComplete and module.isComplete() then
    ...
    if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", "completed task " .. tostring(task.name), "debug", true) end
    queueIndex = (queueIndex or 1) + 1
    advancePastCompletedOrFailed()
    return
  end
  ...
  if task.startTime and (now - task.startTime) > timeout then
    ...
    if task.attempts <= MAX_RETRIES then
      ...
      task.startTime = nil
      if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", string.format("Task '%s' timed out. Re-queueing (attempt %d/%d) in %.1fs.", task.name, task.attempts, MAX_RETRIES, backoff), "info", true) end
    else
      task.failed = true
      task.startTime = nil
      releaseTaskModule(task, false)
      if Log and type(Log.emit) == "function" then pcall(Log.emit, "rfsuite.tasks.onarm", string.format("Task '%s' failed after %d attempts. Skipping.", task.name, MAX_RETRIES), "info", true) end
      queueIndex = (queueIndex or 1) + 1
      advancePastCompletedOrFailed()
    end
  end
