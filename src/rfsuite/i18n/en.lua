return {
  app = {
    ...
    pages = {
      ...
      settings_audio_volume = {
        ...
        save_error_title = "Error",
        save_error_message = "Save failed",
        help_message = "The WAV Volume Level sets a per-callout volume for all RFSuite announcements. 'Radio default' leaves it to the radio's own WAV volume; levels 1 through 5 explicitly set it, with 5 being the loudest. 'Connected only' restricts the override to active flights, so bench and menu work stays at the radio's own level.\n\nThe Master Volume section lets the suite temporarily override the radio's master volume through a global variable. To use it, pick a free GV slot and configure an input, logical switch, and special function in your model:\n\n  1. Inputs: Add input (e.g. I15 'Vol'), Source = GV<n>, weight 100\n  2. Logical Switch: L10: a > x | a = GV<n> | x = -1024\n  3. Special Function: SF: L10 -> Volume = I15 (Vol)\n\nWhile connected, the suite writes the Normal Level to the chosen GV. During a sustained critical alert (low voltage, ESC over-temperature, empty tank) it writes the Alert Level instead. When disconnected or the feature is off, it writes -1024 so that L10 turns off and your regular volume pot regains control."
      },
      ...
      setup_power_preferences = {
        ...
        publish_sensors = "Publish SmFt / SmCp",
        esc_current_limit = "ESC Current Limit",
        value_not_set = "NOT SET",
        help_esc_current_limit = "The current limit the speed controller is set to allow, in amps. It is what the dashboard's ESC load reading is a percentage of. An AM32, Scorpion or YGE controller reports its own limit and fills this in the first time its settings page is opened, so it is set by hand only for a controller that does not. Left at NOT SET, an ESC load tile shows no reading.",
        value_auto = "AUTO",
        ...
        help_publish_sensors = "Publish the remaining fuel and the estimated consumption as the SmFt and SmCp telemetry sensors, for logical switches, special functions and the telemetry screens. The dashboard and the announcements do not need them.",
        help_message = "Configure the model type, the local SmartFuel source, whether SmFt and SmCp are published as sensors, and the current limit the speed controller is set to allow. Nothing here is written to the flight controller: the settings are kept on the radio, per flight controller.",
        saved_title = "Saved",
      ...
      setup_esc_motors = {
        ...
        save_error_invalid_payload = "ESC data could not be built. Re-read the ESC before saving.",
        ...
        help_am32 = "Settings held by the AM32 speed controller itself. They are read from it when the page opens and written back to it on SAVE; the flight controller only passes them through. The radio keeps none of them, except the current limit, which it notes for this flight controller so that the dashboard can show the current as an ESC load percentage.",
        help_scorp = "Settings held by the Scorpion speed controller itself. They are read from it when the page opens and written back to it on SAVE; the flight controller only passes them through. The radio keeps none of them, except the current limit, which it notes for this flight controller so that the dashboard can show the current as an ESC load percentage.",
        help_yge = "Settings held by the YGE speed controller itself. They are read from it when the page opens and written back to it on SAVE; the flight controller only passes them through. The radio keeps none of them, except the current limit, which it notes for this flight controller so that the dashboard can show the current as an ESC load percentage.",
      ...
      setup_wizard = {
        ...
        gov_none = "No governor switch picked yet. Pick one here; the step cannot be continued until you do.",
      ...
      setup_accelerometer = {
        help_p4 = "Checking precedes start. Missing samples keep monitoring active; the completion window starts at the start acknowledgement.",
        help_p5 = "Without a completion notice, success is unconfirmed. The notice explains what was observed.",
        checking_title = "Checking",
        checking_message = "Checking the flight controller before calibration. Keep the model level and still.",
        starting_title = "Starting",
        starting_message = "Requesting calibration. Keep the model level and still.",
        calibration_disconnected = "Connection lost. Keep the model still; reconnect to check the result.",
        calibration_check_failed = "No valid status received. Calibration was not requested. Check the connection.",
        calibration_start_failed = "No start acknowledgement received. Keep the model still and check the controller.",
        calibration_timeout_active = "Calibration started; completion is unconfirmed. Keep the model still, then reload trims and verify the result.",
        calibration_timeout_unseen = "No active calibration observed. Keep the model still and check the controller before another attempt.",
        calibration_unavailable = "Calibration support is unavailable. Check the RFSuite installation.",
        calibration_interrupted = "Calibration monitoring was interrupted. Keep the model still and check the controller.",
        calibration_confirm = "Disarm and hold the model level and still before confirming. Keep it still until completion; saving is automatic.",
        calibration_armed = "Disarm the model before calibrating.",
        calibration_no_acc = "The flight controller reports no accelerometer.",
        calibration_busy = "The flight controller is already calibrating. Keep the model still and wait before trying again.",
        calibration_unconfirmed_title = "Calibration not confirmed",
        calibration_not_started_title = "Calibration not started",
        save_error_title = "Error",
        help_p3 = "Disarm and hold the model level and still before confirming. Keep it still until completion; saving is automatic.",
        title = "Accelerometer",
        roll = "Roll",
        pitch = "Pitch",
        loading_title = "Loading",
      ...
      settings_dashboard_inflight = {
        ...
        check_same_gvar = "Bank and value point at the same variable",
        check_same_channel = "Bank and value point at the same channel",
        ...
        plan_from_page = "It writes the values as they are set on this page, saved or not.",
        ...
        plan_nothing = "The model already carries this setup; nothing to write.",
        ...
        help_setup = "Set up the model writes only what differs from the model, after showing it, and says so when nothing does.",
        ...
        gvar_name_clash = "is not the variable this setup drives"
      },
      ...
      tools_flight_log = {
        ...
        setting_ask_on_connect = "Ask which pack after connecting",
        setting_set_fc_profile = "Set the pack's battery profile",
        ...
        help_p7 = "With the prompt on, the dashboard offers this model's packs in fullscreen once after connecting.",
        help_p8 = "A pick in the prompt can also set the flight controller's battery profile, never while armed."
      },
      logs = {
        title = "Logs",
  ...
  widgets = {
    ...
    escstatus = {
      ...
      bec_undervoltage = "BEC UNDERVOLTAGE",
      bec_overvoltage = "BEC OVERVOLTAGE",
      bec_overcurrent = "BEC OVERCURRENT",
      bec_overheat = "BEC OVERHEAT",
      bec_voltage = "BEC VOLTAGE",
      motor_hot = "MOTOR HOT",
      input_voltage = "INPUT VOLTAGE",
      motor_locked = "MOTOR LOCKED",
      throttle_at_start = "THROTTLE AT START",
      throttle_lost = "THROTTLE LOST",
      throttle = "THROTTLE",
      motor_wiring = "MOTOR WIRING",
      startup = "STARTUP FAILED",
      motor_saturated = "MOTOR SATURATED",
      consumption = "CONSUMPTION",
      capacity_limit = "CAPACITY LIMIT",
      current_limit = "CURRENT LIMIT",
      battery_limit = "BATTERY LIMIT",
      esc_temp_limit = "ESC TEMP LIMIT",
      bec_temp_limit = "BEC TEMP LIMIT",
      rudder_cutoff = "RUDDER CUTOFF",
      operation = "OPERATION FAULT",
      operation_warning = "OPERATION WARNING",
      self_test = "SELF TEST",
      eeprom = "EEPROM",
      watchdog = "WATCHDOG",
      programming_open = "PROGRAMMING OPEN",
      rpm_low = "RPM BELOW LIMIT",
      setpoint_noise = "SETPOINT NOISE"
    },
    governor = {
      ACTIVE = "ACTIVE",
      ARMED = "ARMED",
